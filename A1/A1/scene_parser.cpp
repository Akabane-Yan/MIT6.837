...
void SceneParser::parseMaterials() {
	...
	materials = new Material* [num_materials]; //There was a pair of brackets and that is wrong
	// read in the objects
	int count = 0;
	while (num_materials > count) {
		getToken(token);
		if (!strcmp(token, "PhongMaterial")) { //wrong? changed
...
int SceneParser::getToken(char token[MAX_PARSER_TOKEN_LENGTH]) { //never seen before
